...
n,m,k = map(int,list(input().split())) 		# n-вершин, m-ребер, k-столиц
govs_ind = map(int,list(input().split())) # индексы столиц
orig = set() 							# множество ребер (ребра - кортежи (u, v)) изначально
cities = set(range(1,n+1))			# множество городов
...
for u in govs_ind:
	vis = set()				# множество посещенных вершин (комп связности)
	dfs(u,vis)				# поиск компонент связности
	gov_nods.append(vis)


no_govs = cities.copy()		# множество вершин, без связи и не столицы

ans = 0
for reg in gov_nods:
	no_govs -= reg			# вычитаем из городов все, что есть компонентами связности
	size = len(reg)
	ans += int((size * (size - 1)) / 2) 	# кол-во ребер в полном графе в компоненте связности

size = len(no_govs)
ans += int((size * (size - 1)) / 2)			# кол-во ребер в полном графе всех несвязных вершин
...
ans += len(max_gov) * len(no_govs)		# считаем макс кол-во ребер изолир вершиных с макс комп связности
ans -= len(orig)						# отнимаем изначальные ребра
